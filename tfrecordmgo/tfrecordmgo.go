...
// GetAllPerPage retrieves all records from the db and returns a slice of tenancykit.TFRecord type.
// Records using this DB must have a public id value, expressed either by a bson or json tag
// on the given TFRecord struct.
func (mdb *TFRecordDB) GetAllPerPage(ctx context.Context, order string, orderBy string, page int, responsePerPage int) ([]tenancykit.TFRecord, int, error) {
