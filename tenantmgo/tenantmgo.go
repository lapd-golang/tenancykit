...
func (mdb *TenantDB) Create(ctx context.Context, elem tenancykit.Tenant) error {
	...
	if err := database.C(mdb.col).Insert(query); err != nil {
		mdb.metrics.Emit(metrics.Errorf("Failed to create Tenant record"), metrics.With("collection", mdb.col), metrics.With("query", query), metrics.With("error", err.Error()))
		return err
	}
...
// GetAllPerPage retrieves all records from the db and returns a slice of tenancykit.Tenant type.
// Records using this DB must have a public id value, expressed either by a bson or json tag
// on the given Tenant struct.
func (mdb *TenantDB) GetAllPerPage(ctx context.Context, order string, orderBy string, page int, responsePerPage int) ([]tenancykit.Tenant, int, error) {
...
func (mdb *TenantDB) GetByField(ctx context.Context, key string, value interface{}) (tenancykit.Tenant, error) {
	...
	var item tenancykit.Tenant

	if err := database.C(mdb.col).Find(query).One(&item); err != nil {
		mdb.metrics.Emit(metrics.Errorf("Failed to retrieve all records of Tenant type from db"), metrics.With("query", query), metrics.With("collection", mdb.col), metrics.With("error", err.Error()))
...
func (mdb *TenantDB) Update(ctx context.Context, publicID string, elem tenancykit.Tenant) error {
	...
	if fielder, ok := interface{}(elem).(TenantFields); ok {
	...
	if err := database.C(mdb.col).Update(query, queryData); err != nil {
		mdb.metrics.Emit(metrics.Errorf("Failed to update Tenant record"), metrics.With("collection", mdb.col), metrics.With("query", query), metrics.With("data", queryData), metrics.With("public_id", publicID), metrics.With("error", err.Error()))
		return err
	}
