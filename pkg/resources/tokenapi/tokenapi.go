...
// TokenInfo contains meta-data regarding records in db of
// type pkg.Token.
...
// Info receives an http request to get record info for all available records of type "Token".
//
// Route: /{Route}/info
// Method: INFO
// RESPONSE-BODY: JSON
func (api *HTTPAPI) Info(ctx *httputil.Context) error {
	...
	if err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get pkg.Token record count"), metrics.WithFields(metrics.Field{
			"error": err,
			"url":   ctx.Request().URL.String(),
		}))
	...
	if err := ctx.JSON(http.StatusOK, pkg.TokenInfo{Total: total}); err != nil {
