...
func testInfo(t *testing.T, api httpapi.UserSessionHTTP, db httpapi.UserSessionBackend) {
	tests.Header("When we create a UserSession with UserSessionAPI")
	{
		infoResponse := httptest.NewRecorder()
		infoResource := httptesting.NewRequest("INFO", "/usersession", nil, infoResponse)
		...
		var info httpapi.UserSessionInfo
...
func testCreate(t *testing.T, api httpapi.UserSessionHTTP, db httpapi.UserSessionBackend) {
	tests.Header("When we get info for all UserSession records with UserSessionAPI")
	{
		createResponse := httptest.NewRecorder()
		createResource := httptesting.Post("/usersession", bytes.NewBufferString(fixtures.UserSessionCreateJSON), createResponse)
		if err := api.Create(createResource); err != nil {
			tests.Info("JSON: %+s", fixtures.UserSessionCreateJSON)
		...
		if _, err := fixtures.LoadUserSessionJSON(createResponse.Body.String()); err != nil {
...
func testGetAll(t *testing.T, api httpapi.UserSessionHTTP, db httpapi.UserSessionBackend) {
	tests.Header("When we get all UserSession records with UserSessionAPI")
	{
		...
		getAll := httptesting.Get("/usersession/", nil, getResponse)
		...
		var records httpapi.UserSessionRecords
...
func testGet(t *testing.T, api httpapi.UserSessionHTTP, db httpapi.UserSessionBackend) {
	tests.Header("When we get one UserSession record with UserSessionAPI")
	{
		...
		getResponse := httptest.NewRecorder()
		getRecord := httptesting.Post("/usersession/"+record.PublicID, nil, getResponse)
...
func testUpdate(t *testing.T, api httpapi.UserSessionHTTP, db httpapi.UserSessionBackend) {
	tests.Header("When we update one UserSession record with UserSessionAPI")
	{
		records, total, err := db.GetAll(context.Background(), "", "", 0, 0)
		if err != nil {
			tests.FailedWithError(err, "Should have retrieved all results from backend")
		...
		updateRecord, err := fixtures.LoadUpdateJSON(fixtures.UserSessionUpdateJSON)
		if err != nil {
		...
		getResponse := httptest.NewRecorder()
		getRecord := httptesting.Put("/usersession/"+record.PublicID, bytes.NewBuffer(recordJSON), getResponse)
		getRecord.Bag().Set("public_id", record.PublicID)
...
func testDelete(t *testing.T, api httpapi.UserSessionHTTP, db httpapi.UserSessionBackend) {
	tests.Header("When we delete one UserSession record with UserSessionAPI")
	{
		records, total, err := db.GetAll(context.Background(), "", "", 0, 0)
		if err != nil {
			tests.FailedWithError(err, "Should have retrieved all results from backend")
		...
		getResponse := httptest.NewRecorder()
		getRecord := httptesting.Delete("/usersession/"+record.PublicID, nil, getResponse)
		getRecord.Bag().Set("public_id", record.PublicID)
