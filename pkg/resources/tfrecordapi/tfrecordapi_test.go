...
func TestTFRecordAPI(t *testing.T) {
	logs := metrics.New()
	if testing.Verbose() {
...
func testInfo(t *testing.T, api httpapi.TFRecordHTTP, db httpapi.TFRecordBackend) {
	tests.Header("When we create a TFRecord with TFRecordAPI")
	{
		infoResponse := httptest.NewRecorder()
		infoResource := httptesting.NewRequest("INFO", "/tfrecord", nil, infoResponse)
...
func testCreate(t *testing.T, api httpapi.TFRecordHTTP, db httpapi.TFRecordBackend) {
	tests.Header("When we get info for all TFRecord records with TFRecordAPI")
	{
		createResponse := httptest.NewRecorder()
		createResource := httptesting.Post("/tfrecord", bytes.NewBufferString(fixtures.TFRecordCreateJSON), createResponse)
...
func testGetAll(t *testing.T, api httpapi.TFRecordHTTP, db httpapi.TFRecordBackend) {
	tests.Header("When we get all TFRecord records with TFRecordAPI")
	{
		...
		getAll := httptesting.Get("/tfrecord/", nil, getResponse)
		...
		var records httpapi.TFRecordRecords
...
func testGet(t *testing.T, api httpapi.TFRecordHTTP, db httpapi.TFRecordBackend) {
	tests.Header("When we get one TFRecord record with TFRecordAPI")
	{
		...
		getResponse := httptest.NewRecorder()
		getRecord := httptesting.Post("/tfrecord/"+record.PublicID, nil, getResponse)
...
func testUpdate(t *testing.T, api httpapi.TFRecordHTTP, db httpapi.TFRecordBackend) {
	tests.Header("When we update one TFRecord record with TFRecordAPI")
	{
		records, total, err := db.GetAll(context.Background(), "", "", 0, 0)
		if err != nil {
			tests.FailedWithError(err, "Should have retrieved all results from backend")
		...
		updateRecord, err := fixtures.LoadUpdateJSON(fixtures.TFRecordUpdateJSON)
		if err != nil {
		...
		getResponse := httptest.NewRecorder()
		getRecord := httptesting.Put("/tfrecord/"+record.PublicID, bytes.NewBuffer(recordJSON), getResponse)
		getRecord.Bag().Set("public_id", record.PublicID)
...
func testDelete(t *testing.T, api httpapi.TFRecordHTTP, db httpapi.TFRecordBackend) {
	tests.Header("When we delete one TFRecord record with TFRecordAPI")
	{
		records, total, err := db.GetAll(context.Background(), "", "", 0, 0)
		if err != nil {
			tests.FailedWithError(err, "Should have retrieved all results from backend")
		...
		getResponse := httptest.NewRecorder()
		getRecord := httptesting.Delete("/tfrecord/"+record.PublicID, nil, getResponse)
		getRecord.Bag().Set("public_id", record.PublicID)
