// Package twofactorsessionapi provides a auto-generated package which contains a http restful CRUD API for the specific TwoFactorSession struct in package pkg.
//
//
package twofactorsessionapi
...
// TwoFactorSessionBackend defines an interface which allows the HTTPAPI to divert the final operation of
// the given CRUD request for the "TwoFactorSession" type. This is provided by the user.
// @implement
...
// TwoFactorSessionInfo contains meta-data regarding records in db of
// type pkg.TwoFactorSession.
type TwoFactorSessionInfo struct {
	Total int `json:"total"`
}

// TwoFactorSessionRecords defines a type to represent the response given to a request for
// all records of the type pkg.TwoFactorSession.
type TwoFactorSessionRecords struct {
	Page            int                    `json:"page"`
	ResponsePerPage int                    `json:"responsePerPage"`
	TotalRecords    int                    `json:"total_records"`
	Records         []pkg.TwoFactorSession `json:"records"`
}

// HTTPAPI defines a struct which holds the http api handlers for providing CRUD
// operations for the provided "TwoFactorSession" type.
type HTTPAPI struct {
	metrics  metrics.Metrics
	operator TwoFactorSessionBackend
}

// New returns a new HTTPAPI instance using the provided operator and
// metric.
func New(m metrics.Metrics, backend TwoFactorSessionBackend) *HTTPAPI {
	return &HTTPAPI{
		metrics:  m,
		operator: backend,
	}
...
// Info receives an http request to get record info for all available records of type "TwoFactorSession".
//
// Route: /{Route}/info
// Method: INFO
// RESPONSE-BODY: JSON
func (api *HTTPAPI) Info(ctx *httputil.Context) error {
	...
	if err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get pkg.TwoFactorSession record count"), metrics.WithFields(metrics.Field{
			"error": err,
			"url":   ctx.Request().URL.String(),
		}))
	...
	if err := ctx.JSON(http.StatusOK, pkg.TwoFactorSessionInfo{Total: total}); err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get serialized pkg.TwoFactorSession record to response writer"), metrics.WithFields(metrics.Field{
			"error": err,
			"url":   ctx.Request().URL.String(),
		}))
...
// Create receives an http request to create a new "TwoFactorSession".
//
...
func (api *HTTPAPI) Create(ctx *httputil.Context) error {
	...
	var incoming pkg.TwoFactorSession
...
// Update receives an http request to create a new "TwoFactorSession".
//
// Route: /{Route}/:public_id
// Method: PUT
// BODY: JSON
...
// Delete receives an http request to create a new "TwoFactorSession".
//
// Route: /{Route}/:public_id
// Method: DELETE
//
func (api *HTTPAPI) Delete(ctx *httputil.Context) error {
	...
	if err := api.operator.Delete(ctx.Context(), publicID); err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to delete pkg.TwoFactorSession record"), metrics.WithFields(metrics.Field{
			"error":     err,
			"public_id": publicID,
			"url":       ctx.Request().URL.String(),
		}))
...
// Get receives an http request to create a new "TwoFactorSession".
//
// Route: /{Route}/:public_id
// Method: GET
// RESPONSE-BODY: JSON
func (api *HTTPAPI) Get(ctx *httputil.Context) error {
	...
	if err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get pkg.TwoFactorSession record"), metrics.WithFields(metrics.Field{
			"error":     err,
			"public_id": publicID,
			"url":       ctx.Request().URL.String(),
		}))
...
// GetAll receives an http request to return all "TwoFactorSession" records.
//
// Route: /{Route}/
// Method: GET
// RESPONSE-BODY: JSON
func (api *HTTPAPI) GetAll(ctx *httputil.Context) error {
	...
	if err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get all pkg.TwoFactorSession record"), metrics.WithFields(metrics.Field{
			"error": err,
			"url":   ctx.Request().URL.String(),
		}))
	...
	if err := ctx.JSON(http.StatusOK, TwoFactorSessionRecords{
		Page:            pageNo,
		Records:         requested,
		TotalRecords:    total,
