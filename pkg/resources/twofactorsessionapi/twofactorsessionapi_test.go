...
func testInfo(t *testing.T, api httpapi.TwoFactorSessionHTTP, db httpapi.TwoFactorSessionBackend) {
	tests.Header("When we create a TwoFactorSession with TwoFactorSessionAPI")
	{
		infoResponse := httptest.NewRecorder()
		infoResource := httptesting.NewRequest("INFO", "/twofactorsession", nil, infoResponse)
		...
		var info httpapi.TwoFactorSessionInfo
...
func testCreate(t *testing.T, api httpapi.TwoFactorSessionHTTP, db httpapi.TwoFactorSessionBackend) {
	tests.Header("When we get info for all TwoFactorSession records with TwoFactorSessionAPI")
	{
		createResponse := httptest.NewRecorder()
		createResource := httptesting.Post("/twofactorsession", bytes.NewBufferString(fixtures.TwoFactorSessionCreateJSON), createResponse)
...
func testGetAll(t *testing.T, api httpapi.TwoFactorSessionHTTP, db httpapi.TwoFactorSessionBackend) {
	tests.Header("When we get all TwoFactorSession records with TwoFactorSessionAPI")
	{
		...
		getAll := httptesting.Get("/twofactorsession/", nil, getResponse)
		...
		var records httpapi.TwoFactorSessionRecords
...
func testGet(t *testing.T, api httpapi.TwoFactorSessionHTTP, db httpapi.TwoFactorSessionBackend) {
	tests.Header("When we get one TwoFactorSession record with TwoFactorSessionAPI")
	{
		...
		getResponse := httptest.NewRecorder()
		getRecord := httptesting.Post("/twofactorsession/"+record.PublicID, nil, getResponse)
...
func testUpdate(t *testing.T, api httpapi.TwoFactorSessionHTTP, db httpapi.TwoFactorSessionBackend) {
	tests.Header("When we update one TwoFactorSession record with TwoFactorSessionAPI")
	{
		records, total, err := db.GetAll(context.Background(), "", "", 0, 0)
		if err != nil {
			tests.FailedWithError(err, "Should have retrieved all results from backend")
		...
		updateRecord, err := fixtures.LoadUpdateJSON(fixtures.TwoFactorSessionUpdateJSON)
		if err != nil {
		...
		getResponse := httptest.NewRecorder()
		getRecord := httptesting.Put("/twofactorsession/"+record.PublicID, bytes.NewBuffer(recordJSON), getResponse)
		getRecord.Bag().Set("public_id", record.PublicID)
...
func testDelete(t *testing.T, api httpapi.TwoFactorSessionHTTP, db httpapi.TwoFactorSessionBackend) {
	tests.Header("When we delete one TwoFactorSession record with TwoFactorSessionAPI")
	{
		records, total, err := db.GetAll(context.Background(), "", "", 0, 0)
		if err != nil {
			tests.FailedWithError(err, "Should have retrieved all results from backend")
		...
		getResponse := httptest.NewRecorder()
		getRecord := httptesting.Delete("/twofactorsession/"+record.PublicID, nil, getResponse)
		getRecord.Bag().Set("public_id", record.PublicID)
