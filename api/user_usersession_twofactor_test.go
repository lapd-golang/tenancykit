...
func TestTwoFactorAuthWithMultiTenantUserSession(t *testing.T) {
	m := metrics.New()
	...
	testMultiTenantUserLogin(t, userRecord, userCreateBody, tokenSessionAPI, tf, ufsdb)
}

func TestTwoFactorAuthWithUserSession(t *testing.T) {
	m := metrics.New()
	tfdb := mock.TFRecordBackend()
	ttdb := mock.TenantDBBackend()
	tfsdb := mock.TFSessionBackend()
...
func testMultiTenantUserLogin(t *testing.T, user pkg.User, usercreate pkg.CreateUser, tsession api.TwoFactorSessionAPI, tf api.MultiTenantUserSessionAPI, db types.UserSessionDBBackend) {
	tests.Header("When login and logging out using the UserSessionAPI")
	{
		var login pkg.CreateUserSession
		login.Email = user.Email
