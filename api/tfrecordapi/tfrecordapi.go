// Package tfrecordapi provides a auto-generated package which contains a http restful CRUD API for the specific TFRecord struct in package tenancykit.
//
//
package tfrecordapi
...
// TFRecordBackend defines an interface which allows the HTTPAPI to divert the final operation of
...
// all records of the type tenancykit.TFRecord.
type TFRecordRecords struct {
	Page            int                   `json:"page"`
	ResponsePerPage int                   `json:"responsePerPage"`
	TotalRecords    int                   `json:"total_records"`
	Records         []tenancykit.TFRecord `json:"records"`
}

// HTTPAPI defines a struct which holds the http api handlers for providing CRUD
// operations for the provided "TFRecord" type.
type HTTPAPI struct {
	metrics  metrics.Metrics
	operator TFRecordBackend
}

// New returns a new HTTPAPI instance using the provided operator and
// metric.
func New(m metrics.Metrics, backend TFRecordBackend) *HTTPAPI {
	return &HTTPAPI{
		metrics:  m,
		operator: backend,
	}
...
// Update receives an http request to create a new "TFRecord".
//
// Route: /{Route}/:public_id
// Method: PUT
// BODY: JSON
...
func (api *HTTPAPI) Update(ctx *httputil.Context) error {
	...
	var incoming tenancykit.TFRecord
...
// Delete receives an http request to create a new "TFRecord".
//
// Route: /{Route}/:public_id
// Method: DELETE
//
func (api *HTTPAPI) Delete(ctx *httputil.Context) error {
	...
	if err := api.operator.Delete(ctx, publicID); err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to delete tenancykit.TFRecord record"), metrics.WithFields(metrics.Field{
			"error":     err,
			"public_id": publicID,
			"url":       ctx.Request().URL.String(),
		}))
...
// Get receives an http request to create a new "TFRecord".
//
// Route: /{Route}/:public_id
// Method: GET
// RESPONSE-BODY: JSON
func (api *HTTPAPI) Get(ctx *httputil.Context) error {
	...
	if err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get tenancykit.TFRecord record"), metrics.WithFields(metrics.Field{
			"error":     err,
			"public_id": publicID,
			"url":       ctx.Request().URL.String(),
		}))
	...
	if err := ctx.JSON(http.StatusOK, requested); err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get serialized tenancykit.TFRecord record to response writer"), metrics.WithFields(metrics.Field{
			"error":     err,
			"public_id": publicID,
			"url":       ctx.Request().URL.String(),
		}))
...
// GetAll receives an http request to return all "TFRecord" records.
//
// Route: /{Route}/
// Method: GET
// RESPONSE-BODY: JSON
func (api *HTTPAPI) GetAll(ctx *httputil.Context) error {
	...
	if err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get all tenancykit.TFRecord record"), metrics.WithFields(metrics.Field{
			"error": err,
			"url":   ctx.Request().URL.String(),
		}))
	...
	if err := ctx.JSON(http.StatusOK, TFRecordRecords{
		Page:            pageNo,
		Records:         requested,
		TotalRecords:    total,
