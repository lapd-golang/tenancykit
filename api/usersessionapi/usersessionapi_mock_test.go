...
// GetAll returns a slice of all available record of type tenancykit.UserSession.
func (m *MockAPIOperator) GetAll(ctx context.Context, order string, orderBy string, page int, responserPerPage int) ([]tenancykit.UserSession, int, error) {
	var records []tenancykit.UserSession

	m.store.Range(func(k, v interface{}) bool {
		if elem, ok := v.(tenancykit.UserSession); ok {
...
func (m *MockAPIOperator) Get(ctx context.Context, publicID string) (tenancykit.UserSession, error) {
	elem, found := m.store.Load(publicID)
	if !found {
		return tenancykit.UserSession{}, fmt.Errorf("Record does not exists with id %q", publicID)
	}

	rElem, ok := elem.(tenancykit.UserSession)
	if !ok {
		return tenancykit.UserSession{}, errors.New("Record does not match type")
	}
...
func (m *MockAPIOperator) Update(ctx context.Context, publicID string, elem tenancykit.UserSession) error {
...
func (m *MockAPIOperator) Create(ctx context.Context, elem tenancykit.UserSession) (tenancykit.UserSession, error) {
