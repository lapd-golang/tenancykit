...
// GetAll returns a slice of all available record of type tenancykit.TwoFactorSession.
func (m *MockAPIOperator) GetAll(ctx context.Context, order string, orderBy string, page int, responserPerPage int) ([]tenancykit.TwoFactorSession, int, error) {
	var records []tenancykit.TwoFactorSession

	m.store.Range(func(k, v interface{}) bool {
		if elem, ok := v.(tenancykit.TwoFactorSession); ok {
...
func (m *MockAPIOperator) Get(ctx context.Context, publicID string) (tenancykit.TwoFactorSession, error) {
	elem, found := m.store.Load(publicID)
	if !found {
		return tenancykit.TwoFactorSession{}, fmt.Errorf("Record does not exists with id %q", publicID)
	}

	rElem, ok := elem.(tenancykit.TwoFactorSession)
	if !ok {
		return tenancykit.TwoFactorSession{}, errors.New("Record does not match type")
	}
...
func (m *MockAPIOperator) Update(ctx context.Context, publicID string, elem tenancykit.TwoFactorSession) error {
...
func (m *MockAPIOperator) Create(ctx context.Context, elem tenancykit.TwoFactorSession) (tenancykit.TwoFactorSession, error) {
