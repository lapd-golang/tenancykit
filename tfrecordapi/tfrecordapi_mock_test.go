...
// GetAll returns a slice of all available record of type tenancykit.TFRecord.
func (m *MockAPIOperator) GetAll(ctx context.Context, order string, orderBy string, page int, responserPerPage int) ([]tenancykit.TFRecord, int, error) {
	var records []tenancykit.TFRecord

	m.store.Range(func(k, v interface{}) bool {
		if elem, ok := v.(tenancykit.TFRecord); ok {
...
func (m *MockAPIOperator) Get(ctx context.Context, publicID string) (tenancykit.TFRecord, error) {
	elem, found := m.store.Load(publicID)
	if !found {
		return tenancykit.TFRecord{}, fmt.Errorf("Record does not exists with id %q", publicID)
	}

	rElem, ok := elem.(tenancykit.TFRecord)
	if !ok {
		return tenancykit.TFRecord{}, errors.New("Record does not match type")
	}
...
func (m *MockAPIOperator) Update(ctx context.Context, publicID string, elem tenancykit.TFRecord) error {
...
func (m *MockAPIOperator) Create(ctx context.Context, elem tenancykit.NewTF) (tenancykit.TFRecord, error) {
...
// createTFRecord creates a new tenancykit.TFRecord record from the given create type.
func createTFRecord(ctx context.Context, elem tenancykit.NewTF) (tenancykit.TFRecord, error) {

	// TODO(developer):
	// Override function contents with what should happen.

	return tenancykit.TFRecord{}, errors.New("Not Implemented")
}
