...
// all records of the type users.User.
...
func (api *HTTPAPI) Delete(ctx *httputil.Context) error {
	...
	if err := api.operator.Delete(ctx, publicID); err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to delete users.User record"), metrics.WithFields(metrics.Field{
			"error":     err,
			"public_id": publicID,
			"url":       ctx.Request().URL.String(),
		}))
...
func (api *HTTPAPI) Get(ctx *httputil.Context) error {
	...
	if err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get users.User record"), metrics.WithFields(metrics.Field{
			"error":     err,
			"public_id": publicID,
			"url":       ctx.Request().URL.String(),
		}))
	...
	if err := ctx.JSON(http.StatusOK, requested); err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get serialized users.User record to response writer"), metrics.WithFields(metrics.Field{
			"error":     err,
			"public_id": publicID,
			"url":       ctx.Request().URL.String(),
		}))
...
func (api *HTTPAPI) GetAll(ctx *httputil.Context) error {
	...
	if err != nil {
		api.metrics.Emit(metrics.Errorf("Failed to get all users.User record"), metrics.WithFields(metrics.Field{
			"error": err,
			"url":   ctx.Request().URL.String(),
		}))
